...
class ProductApp(ct.CTkToplevel):  # Usamos CTkToplevel para permitir volver a dashboard
    ...
    def __init__(self, master):  # Constructor
        ...
        ct.CTkButton(btn_frame, text="Actualizar", command=self.apply_product_update,
                     fg_color="#007BFF", hover_color="#0056b3", text_color="white", width=120).grid(row=0, column=2, padx=5)

        ct.CTkButton(btn_frame, text="Eliminar", command=self.delete_product,
                     fg_color="#E63946", hover_color="#C53030", text_color="white", width=120).grid(row=0, column=3, padx=5)

        ct.CTkButton(btn_frame, text="Borrar Campos", command=self.clear_entries,
                     fg_color="#6C757D", hover_color="#5A6268", text_color="white", width=120).grid(row=0, column=4, padx=5)

        ct.CTkButton(btn_frame, text="Regresar", command=self.back_to_dashboard,
                     fg_color="#F4A261", hover_color="#E76F51", text_color="white", width=120).grid(row=0, column=5, padx=5)
    ...
    def update_product(self):  # Cargar datos del producto seleccionado
    ...
    def apply_product_update(self):  # Método para aplicar cambios
        if self.current_id:
